...
anta_prod_renewable_group <- function(production_dir = NULL){
  ...
  data <- ref_join_class(x = data, classobj= "on_ctry_dates_prod_type", date_time = time_vars, global_options$renewable_production_per_country )

  data$observed[is.na(data$observed)] <- FALSE

  data <- as.data.frame(data)

  class(data) <- c(class(data), "raw_prod_renewable_group" )
